...
if [ -z "$(ls -A "$datasets_dir")" ]; then
    ...
    "$SCRIPT_DIR/scripts/download_datasets.sh" -l "$datasets_dir" -f yolov8
fi

# Prepare the results directory
dir="$SCRIPT_DIR/runs/yolo-v8"
mkdir -p "$dir"
...
wget -nc https://github.com/ultralytics/assets/releases/download/v8.3.0/yolov8s.pt
...
echo "Done training all the datasets with YOLOv8!"
