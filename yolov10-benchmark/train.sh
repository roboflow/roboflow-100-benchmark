...
if [ -z "$(ls -A "$datasets_dir")" ]; then
    ...
    "$SCRIPT_DIR/scripts/download_datasets.sh" -l "$datasets_dir" -f yolov10
fi

# Prepare the results directory
dir="$SCRIPT_DIR/runs/yolo-v10"
mkdir -p "$dir"
if [ ! -f "$dir/final_eval.txt" ]; then
    touch "$dir/final_eval.txt"
fi

cd "$SCRIPT_DIR/yolov10-benchmark/"
...
wget -nc https://github.com/ultralytics/assets/releases/download/v8.3.0/yolov10s.pt
echo "Model file downloaded."

# Set the model path
model_path="$SCRIPT_DIR/yolov10-benchmark/yolov10s.pt"
        ...
        for ((gpu_id=0; gpu_id<num_gpus; gpu_id++)); do
            ...
            if flock -n "$lock_fd"; then
                ...
                if [ ! -f "$results_dir/train/weights/best.pt" ]; then
                    ...
                    python3 "$SCRIPT_DIR/yolov10-benchmark/parse_eval.py" -d "$dataset_name" -r "$results_dir/train" -o "$dir/final_eval.txt"
                else
...
echo "Done training all the datasets with YOLOv10!"
